 ...
 *
 * No es un ECS data-oriented puro (DOTS/archetypes) — YAGNI (§11.1), pero
 * la API es compatible con migrar a uno más estricto si en el futuro se
 * necesitan millones de entidades.
 */
...
export class World {
   ...
   * Operación O(n * k) donde k = número de componentes consultados.
   * Para queries frecuentes se debería cachear (futuro YAGNI).
   */
  ...
  *iter<T>(name: string): IterableIterator<[EntityId, T]> {
  ...
  // === Etiquetas (única entidad con ese nombre, ej. "player") ===
